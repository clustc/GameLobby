/**
 * 备注:
 * 1,第一次进入游戏，需设置搜索路径为大厅更新目录（大厅更新目录写死：(jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + 'hall'）；其他则需设置为缓存的loadPath字段；
 * 2,大厅进子游戏时，需设置缓存loadPath字段为子游戏下载目录 (jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + subGameName;
 * 3,子游戏回切回大厅时，初始场景需置为大厅场景
 */
(function () { 

    //更新重启加载路径跟换；大厅子游戏切换逻辑
    if(CacheData){
        let searchPath =  jsb.fileUtils.getSearchPaths();
        let path = CacheData.getInstance().get("loadPath");
        if(!path){
            path = (jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + 'hall';
        }
        searchPath.unshift(path);
    ...
    function boot () {
        ...
        var onStart = function () {
            ...
            //初始加载场景判断，如果是从子游戏切换大厅，需修改初始场景为大厅场景
            if(CacheData){
                let path = CacheData.getInstance().get('loadPath');
                if(path.indexOf('/hall') > -1){
                    launchScene = 'GameLobbyScene';
                }
            }
